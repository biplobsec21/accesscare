<div class="modal fade" id="Modal{{ $document->id }}" tabindex="-1" role="dialog" aria-hidden="true">
	<form method="post" action="{{ route('eac.portal.drug.doc.modal.update',$document->id) }}"
     ...
     <div class="row">
      ...
      <div class="col-lg-auto mb-3">
       ...
       <div class="row">
        <div class="col">
         <a href="{{ route('eac.portal.file.download', $document->file_id) }}" class="btn btn-link" data-toggle="tooltip" data-placement="bottom" title="Download File">
          <i class="far fa-download text-muted"></i> {!! $document->file->name !!}
         </a>
        </div>
        <div class="col-auto">
         <a class="text-danger" href="#" onclick="removeTemplate('{{$document->id}}', event)" data-toggle="tooltip" data-placement="bottom" title="Delete File">
          <i class="fal fa-times"></i> <span class="sr-only">Delete</span>
         </a>
        </div>
       </div>
						@else
