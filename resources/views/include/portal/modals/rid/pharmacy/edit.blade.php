<div class="modal fade" id="PharmacyModal{{ $shipment->id }}" tabindex="-1" role="dialog" aria-hidden="true">
						...
						<select id="pharmacySelect" name="pharmacy_id" class="select2 form-control">
							...
							@foreach($pharmacies as $pharmacy)
								<option value="{{ $pharmacy->id }}"
										@if($pharmacy->id == ($shipment->pharmacy->id ?? null)) selected @endif>{{ $pharmacy->name }}</option>
							@endforeach
						</select>
					</div>
					<div id="oldPharmacy" class="collapse show mb-3">
						<div id="pharmaInfo" class="small">
							...
							<select id="pharmacistSelect" name="pharmacist_id" class="form-control">
								@if($pharmacy->pharmacists->count() == 0)
									<option value="">No Pharmacists</option>
								@else
