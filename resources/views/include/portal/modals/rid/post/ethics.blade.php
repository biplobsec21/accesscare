<div class="modal fade" id="EthicsModal{{ $rid->id }}" tabindex="-1" role="dialog" aria-hidden="true">
...
<div class="modal fade" id="ReviewEthicsModal{{ $rid->id }}" tabindex="-1" role="dialog" aria-hidden="true">
 <form method="post" action="{{ route('eac.portal.rid.modal.user.save') }}" enctype="multipart/form-data">
  {{ csrf_field() }}
  <input type="hidden" name="id" value="{{ $rid->id }}"/>
  <div class="modal-dialog modal-dialog-centered" role="document">
    ...
    <div class="modal-body p-3">
     <div class="mb-3">
      <div class="row">
       ...
       <div class="col-sm">
        Does the IRB / Ethics Approval document match with the corresponding drug, physician and patient?
       </div>
      </div>
     </div>
     <hr class="mt-0" />
