...
@section('content')
	...
	<div class="actionBar">
		...
		<div class="btn-group" role="group">
			<button id="btnGroupDrop3" type="button" class="btn btn-primary" data-toggle="dropdown" aria-haspopup="true"
					aria-expanded="false">
			...
			<div class="dropdown-menu" aria-labelledby="btnGroupDrop3">
				@foreach(\App\RidMasterStatus::all() as $masterStatus)
					<a class="dropdown-item"
					   href="{{route('eac.portal.rid.changestatus', ['rid_id' => $rid->id,'status' => $masterStatus->id])}}">{{ $masterStatus->name}}</a>
				@endforeach
		...
		<div class="row thisone m-0 mb-xl-5">
			<div class="col-sm-3 col-xl-auto p-0 pr-sm-2 mb-2 mb-sm-0">
				<div class="wizardSteps symbols nav flex-row flex-sm-column" id="tab" role="tablist"
					...
					<a class="nav-link active complete " id="xpatientT-tab" data-toggle="pill" href="#xpatientT"
					   role="tab" aria-controls="xpatientT" aria-selected="true">
						<span>Patient Information</span>
					</a>
					<a class="nav-link complete" id="xdrugT-tab" data-toggle="pill" href="#xdrugT" role="tab"
					   aria-controls="xdrugT" aria-selected="false">
						<span>Drug Information</span>
					</a>
					<a class="nav-link complete" id="xusergrpT-tab" data-toggle="pill" href="#xusergrpT" role="tab"
					   aria-controls="xusergrpT" aria-selected="false">
						<span>User Groups</span>
					</a>
					<a class="nav-link  @if($rid->notes->count() > 0) complete @endif " id="xnotesT"
			...
			<div class="col-sm-9 col-xl p-0 pl-sm-2">
				<div class="card tab-content wizardContent" id="tabContent">
					...
					<div class="tab-pane fade show active" id="xpatientT" role="tabpanel"
						 aria-labelledby="xpatientT-tab">
					...
					<div class="tab-pane fade" id="xnotes" role="tabpanel" aria-labelledby="xnotes-tab">
						<div class="card-body">
							...
							<h5 class="text-gold mb-2">
								Notes &amp; Correspondence <span class="badge badge-dark">{{$rid->notes->count()}}</span>
							</h5>
							<div class="row">
								<div class="col">
					...
					@if(true)
						<div class="bg-gradient-dark p-3 d-flex justify-content-between">
							<a class="btn btn-danger" title="Deny Rid"
							   href="{{route('eac.portal.rid.deny', $rid->id)}}">
								<i class="fas d-sm-inline fa-ban"></i> Not Approved
							</a>
							...
							<a class="btn btn-success" title="Approve Rid"
							   href="{{route('eac.portal.rid.approve', $rid->id)}}">
								<i class="fas d-sm-inline fa-check"></i> Approve Request
							</a>
						</div>
					@endif
				</div>
