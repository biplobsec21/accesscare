...
@section('content')
		...
		@foreach($ridPADocs as $document)
			...
			@if($document->type == 'Uploadable')
				...
				@if($doc->uploaded_file_id ?? null)
					<a title="Add" class="btn btn-link text-success" data-toggle="modal" data-target="#UploadModal{{$document->id}}" href="#UploadModal{{$document->id}}" aria-hidden="true">
						<i class="far fa-plus"></i> Uploaded (Click to change)
					</a>
					<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#UploadReviewModal{{$document->id}}" href="#UploadReviewModal{{$document->id}}" aria-hidden="true">
						Review
					</a>
				@else
					<a title="Add" class="btn btn-link text-danger" data-toggle="modal" data-target="#UploadModal{{$document->id}}" href="#UploadModal{{$document->id}}" aria-hidden="true">
						<i class="far fa-plus"></i> Upload required
					</a>
				@endif
          ...
          <label class="d-block">Attach Document <small>({{config('eac.storage.file.type')}})</small></label>
										...
										@else
           <label class="d-block mt-1"><small>Maximum filesize: {{config('eac.storage.file.maxSize')}}</small></label>
    ...
    <h5 class="mb-0">{{$document->name}}</h5>
     ...
     <div class="text-success">
      <i class="far fa-check"></i> Acknowledgement Complete
     </div>
				@else
     <a title="Add" class="btn btn-link text-danger" data-toggle="modal" data-target="#AcknowledgeModal{{$document->id}}" href="#AcknowledgeModal{{$document->id}}" aria-hidden="true">
      <i class="far fa-plus"></i> Acknowledgement Required
					</a>
				@endif				
