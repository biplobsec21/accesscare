			...
			<div class="col-lg-10 col-xl-10">
				@foreach($ridPADocs as $document)
					...
					@if($document->type == 'Uploadable')
							...
							<div class="col-md-6 card card-body mb-0">
								@if($doc && $doc->uploaded_file_id)
									<a title="Add" class="upload-add" data-toggle="modal" data-target="#UploadModal{{$document->id}}" aria-hidden="true">
										<i class="far fa-plus text-success" ></i> <span class="text-success">Uploaded (Click to change)</span>
									</a>
									<a href="#" class="btn btn-primary" data-toggle="modal"
									   data-target="#UploadReviewModal{{$document->id}}" aria-hidden="true">
										Review
									</a>
								@else
									<a title="Add" class="upload-add" data-toggle="modal" data-target="#UploadModal{{$document->id}}" aria-hidden="true">
										<i class="far fa-plus text-danger" ></i> <span class="text-danger">Upload required</span>
									</a>
								@endif

							</div>
						</div>
						<div class="modal fade" id="UploadModal{{$document->id}}" tabindex="-1" role="dialog"
							 aria-hidden="true">
							<form method="post" action="{{ route('eac.portal.rid.modal.review.doc.save') }}"
										...
										<div class="modal-body p-3">
											<div class="mb-3">
													...
													<div class="row">
														...
														<div class="col-sm-auto">
															<a class="btn btn-danger btn-sm" href="#"
															   onclick="removeDocument('{{$doc->id}}',event)">
												...
												<div class="input-group">
													<input type="file" class="form-control" name="upload_file"
														   value="filename.extension" required/>
												</div>
												<?php } ?>
											...
											<div class="">
												...
												<textarea class="form-control" rows="3"
														  name="up_notes">{{($doc && $doc->upload_notes) ? $doc->upload_notes : ''}}</textarea>
											</div>
										</div>
						...
						<div class="modal fade" id="UploadReviewModal{{$document->id}}" tabindex="-1" role="dialog"
							 aria-hidden="true">
							<form method="post" action="{{ route('eac.portal.rid.modal.review.doc.save') }}"
								  enctype="multipart/form-data">
								{{ csrf_field() }}
								...
								<div class="modal-dialog modal-dialog-centered" role="document">
									<div class="modal-content">
										...
										<div class="alert-secondary p-2 border-bottom text-center">
											<a href="{{($doc && $doc->uploaded_file_id) ? route('eac.portal.file.download', $file->id) : '#'}}">
										...
										<div class="modal-body p-3">
												...
												<div class="row">
													<div class="col-sm-auto">
														<label class="d-sm-block mb-1">
															<input type="radio" name="is_match"
																   value="yes" {{($doc && $doc->is_matched == 'yes') ? 'checked' : ''}}/>
															Yes
														</label>
														<label class="d-sm-block">
															<input type="radio" name="is_match"
																   value="no" {{($doc && $doc->is_matched == 'no') ? 'checked' : ''}}/>
															No
														</label>
													</div>
													<div class="col-sm">
														Does the {{$document->name}} document match with the
														corresponding drug, physician and patient?
													</div>
												</div>
											</div>
											<hr class="mt-0"/>
											<div class="row">
												<div class="col-md mb-3">
													...
													<input type="number"
														   value="{{($doc && $doc->record_no) ? $doc->record_no : ''}}"
														   name="record_no" class="form-control"/>
												</div>
												<div class="col-md mb-3">
													...
													<input type="text"
														   value="{{($doc && $doc->expiration_date) ? $doc->expiration_date : ''}}"
											...
											<div class="">
												<label class="d-block">Notes</label>
												<textarea class="form-control" rows="3"
														  name="review_notes">{{($doc && $doc->review_notes) ? $doc->review_notes : ''}}</textarea>
											</div>
										</div>
										<div class="modal-footer p-2 d-flex justify-content-between">
											<button type="button" class="btn btn-secondary" data-dismiss="modal"
							...
							<div class="col-md-6 card card-body mb-0">
								...
								@else
									<a title="Add" class="upload-add" data-toggle="modal" data-target="#AcknowledgeModal{{$document->id}}" aria-hidden="true">
										<i class="far fa-plus text-danger" ></i><span class="text-danger">Acknowledgement Required </span>
									</a>
								@endif
							</div>
						</div>

						<div class="modal fade" id="AcknowledgeModal{{$document->id}}" tabindex="-1" role="dialog"
							 aria-hidden="true">
							<form method="post" action="{{ route('eac.portal.rid.modal.review.doc.save') }}"
								  enctype="multipart/form-data">
								{{ csrf_field() }}
