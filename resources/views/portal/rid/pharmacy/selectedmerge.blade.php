...
@include('include.alerts')
	...
	<div class="viewData">
  <form method="post" action="{{ route('eac.portal.pharmacy.list.mergepost') }}">
    {{ csrf_field() }}
     ...
     @if($rowsPrimary->count() > 0)
       @foreach($rowsPrimary as $val)
         <div class="card card-body mb-1 pt-2">
          ...
          <p class="text-muted m-0">
           <span class="strong">Country:</span> {{ $val->address && $val->address->country && $val->address->country->name ? $val->address->country->name : ''}}
          </p>
      ...
      @if($rowsMerge->count() > 0)
       @foreach($rowsMerge as $val)
         <div class="card card-body mb-1 pt-2">
          ...
          <p class="text-muted m-0">
           <span class="strong">Country:</span> {{ $val->address->country->name }}
          </p>
