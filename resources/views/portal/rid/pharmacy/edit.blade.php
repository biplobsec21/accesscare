...
@section('content')
            ...
            <div class="row thisone m-0 mb-xl-5">
                <div class="col-sm-3 col-xl-auto mb-2 mb-sm-0 p-0">
                    <div class="wizardSteps symbols nav flex-row flex-sm-column mt-sm-3" id="tab" role="tablist" aria-orientation="vertical">
                        <a class="nav-link active complete" id="xdetailsT" data-toggle="pill" href="#xdetails" role="tab" aria-controls="xdetails" aria-selected="true">
                            <span>Details</span>
                        </a>
                        <a class="nav-link @if($pharmacy->pharmacists->count()) complete @endif" id="xpharmacistsT" data-toggle="pill" href="#xpharmacists" role="tab" aria-controls="xpharmacists" aria-selected="false">
                    ...
                    <div class="card tab-content wizardContent" id="tabContent">
                            ...
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-sm mb-3">
                                        ...
                                        <input type="text" name="pharmacy_name" value="{{ $pharmacy->name }}" class="form-control" required="required">
                                    </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="row m-md-0">
                                        <div class="col-md mb-1 mb-md-3 p-md-0">
                                            <input type="text" name="pharmacy_addr1" value="{{ $pharmacy->address->addr1 ? $pharmacy->address->addr1 : '' }}" class="form-control" placeholder="Street Address">
                                        </div>
                                        <div class="col-md-5 mb-3 p-md-0 pl-md-1">
                                            <input type="text" name="pharmacy_addr2" value="{{ $pharmacy->address->addr2 ?  $pharmacy->address->addr2 : ''}}" class="form-control" placeholder="Address Continued (Building #, Suite, Floor, etc)">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm col-md-8 col-lg-12 col-xl-7 mb-3">
                                        ...
                                        <select name="pharmacy_country_id" id="ci" class="form-control" required="required">
                                    ...
                                    <div class="col-sm col-md-4 col-lg-12 col-xl mb-3">
                                        ...
                                        <input type="text" name="pharmacy_city" value="{{ $pharmacy->address ? $pharmacy->address->city : ''}}" class="form-control" required="required">
                                    </div>
                                </div>
                                <div class="row">
                                        ...
                                        <select name="pharmacy_state_province" id="pharmacy_state_province" class="form-control">
                                            ...
                                            @foreach($states as $state)
                                                @if($pharmacy->address->state_province)
                                                    <option value="{{ $state->id }}" {{$pharmacy->address && $pharmacy->address->state_province == $state->id ? 'selected' : ''}}>{{$state->name}}</option>
                                                @else
                                    ...
                                    <div class="col-sm col-lg-5 mb-3">
                                        ...
                                        <input type="number" name="pharmacy_zip" value="{{ $pharmacy->address ? $pharmacy->address->zipcode : '' }}" class="form-control" required="required">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="tab-pane fade" id="xpharmacists" role="tabpanel" aria-labelledby="xpharmacists-tab">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table cusGem table-sm table-striped table-hover">
                                            ...
                                            @foreach($pharmacy->pharmacists as $val)
                                                <tr>
                                                    <td>
                                                        <a href="{{ route('eac.portal.pharmacist.edit', $val->id) }}" data-toggle="tooltip" title="Edit {{ $val->name}}">
                                                            {{ $val->name}}
                                                        </a>
                                                    </td>
                                                    <td class="small">
                                                    ...
                                                    <td class="text-right">
                                                        <a href="#" onclick="Confirm_Delete('{{ $val->id }}')" class="text-danger" title="Remove Pharmacist">
                            ...
                            <div class="card-footer d-flex justify-content-between p-2 align-items-center">
                                <a href="#" class="btn btn-success window-btn" data-toggle="modal" data-target="#newPharmacistmodal">
                                    Add New Pharmacist
                                </a>
                                <a href="#" class="btn btn-primary" data-toggle="modal" data-target="#pharmacistaddmodal">
                                    Assign Pharmacist
                                </a>
                            </div>
                        </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header p-2">
                        <h5 class="m-0">Add Pharmacist(s)</h5>
                    ...
                    <div class="modal-body p-3">
                        <div class="table-responsive">
                            ...
                            <table class="table table-sm table-striped table-hover fixed_header" id="pharmacistTBL">
                                <tbody>
                                @foreach(App\Pharmacist::whereNull('pharmacy_id')->orderBy('name')->get() as $val)
                                    <tr>
                                        <td>
                                            <input type="checkbox" name="pharmacists[]" value="{{$val->id}}"/>
                                        </td>
                                        <td>
                                            <a href="{{ route('eac.portal.pharmacist.edit', $val->id) }}" data-toggle="tooltip" title="Edit {{ $val->name}}">
                                                {{ $val->name}}
                                        ...
                                        <td>
                                            {{ $val->created_at->format(config('eac.date_format'))}}
                                        </td>
                                    </tr>
                                @endforeach
                                </tbody>
    ...
    <div class="modal fade" id="newPharmacistmodal" tabindex="-1" role="dialog" aria-hidden="true">
        <form method="post" action="{{ route('eac.portal.pharmacy.newpharmacist') }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            <input type="hidden" name="pharmacy_id" value="{{$pharmacy->id}}"/>
            <input type="hidden" name="country_name" value="{{$pharmacy->address && $pharmacy->address->country ? $pharmacy->address->country->id  : 0 }}"/>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body p-3">
                        <table class="table table-sm table-striped table-hover">
                            ...
                            <tr class="">
                                <td>
                                    <input type="text" name="name[]" placeholder="Pharmacist Name" class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}">
                                </td>
                                <td>
    ...
    <table>
        <tr class="group-member group-member-template">
            <td>
                <input type="text" name="name[]" placeholder="Pharmacist Name" class="form-control " required="required">
            </td>
            <td>
