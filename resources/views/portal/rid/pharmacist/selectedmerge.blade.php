...
@include('include.alerts')
    ...
    <a href="{{ route('eac.portal.pharmacist.list.merge') }}" class="btn btn-primary">
      <i class="fal fa-code-merge"></i> List Merge pharmacist
    </a>
	</div><!-- end .actionBar -->

	<div class="viewData">
  <form method="post" action="{{ route('eac.portal.pharmacist.list.mergepost') }}">
    {{ csrf_field() }}
     ...
     @if($rowsPrimary->count() > 0)
       @foreach($rowsPrimary as $val)
         <div class="card card-body mb-1 pt-2">
          ...
          <p class="text-muted m-0">
           <span class="strong">Email:</span> {{ $val->email ? $val->email : 'N/A' }}
          </p>
          <p class="text-muted m-0">
           <span class="strong">Phone:</span> {{ $val->getPhone() && $val->getPhone() ? $val->getPhone() : 'N/A' }}
          </p>
          <p class="text-muted m-0">
           <span class="strong">Pharmacy:</span> {{ $val->pharmacy ? $val->pharmacy->name : 'Not Assigned' }}
          </p>
