...
@section('scripts')
 ...
 $(document).ready(function () {
  ...
  var dataTable = $('#routeTBL').DataTable({
   ...
   'order': [[0, 'asc']],
   "ajax": {
    url: "{{ route('eac.portal.settings.manage.drug.dosage.route.ajaxlist') }}",
    type: "post"
   },
   "processing": true,
   "serverSide": true,
  ...
  swal({
   ...
   if (isConfirm) {
    swal({
     ...
     $.get("{{route('eac.portal.settings.manage.drug.dosage.routedelete')}}",
       {
        id: param
       });
     // return false;
     ...
     $(location).attr('href', '{{route('eac.portal.settings.manage.drug.dosage.route.index')}}') // <--- submit form programmatically
    });
   } else {
