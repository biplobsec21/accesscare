...
@section('content')
 <div class="titleBar">
  ...
  <h2 class="m-0">
   Concentration Unit Manager <small>@yield('title')</small>
  </h2>
 </div><!-- end .titleBar -->
 @php
     ...
     <tbody>
      @if($logData)
       @foreach($logData as $val)
        <tr data-id="{{$val->id}}">
         <td>
          {{  \App\DosageUnit::where('id','=',$val->id)->firstOrFail()->name  }}
         </td>
         <td class="text-center">
          <span class="badge badge-secondary">{{ $val->total_action }}</span>
         </td>
         <td>
          {{  \App\DosageUnit::where('id','=',$val->id)->firstOrFail()->updated_at  }}
         </td>
         <td>
