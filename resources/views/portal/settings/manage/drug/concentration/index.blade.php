...
@section('content')
 ...
 <div class="actionBar">
  <a href="{{ route($page['createButton']) }}" class="btn btn-success">
   <i class="fal fa-eye-dropper"></i> Add New 
  </a>
 ...
 <div class="viewData">
  <div class="card mb-1 mb-md-4">
   ...
   <div class="table-responsive">
    <table class="table table-sm table-striped table-hover" id="concentrationTBL">
     <thead>
      <tr>
       <th>Concentration</th>
       <th>Action</th>
...
@section('scripts')
 ...
 $(document).ready(function () {
  $('#concentrationTBL tfoot th').each(function () {
  ...
  var dataTable = $('#concentrationTBL').DataTable({
   "paginationDefault": 10,
    "paginationOptions": [10, 25, 50, 75, 100],
   // "responsive": true,
   'order': [[0, 'asc']],
   "ajax": {
    url: "{{ route('eac.portal.settings.manage.drug.dosage.concentration.ajaxlist') }}",
    type: "post"
   },
   "processing": true,
   "serverSide": true,
  ...
  swal({
   ...
   if (isConfirm) {
    swal({
     ...
     $.get("{{route('eac.portal.settings.manage.drug.dosage.concentrationdelete')}}",
       {
        id: param
       });
     // return false;
     ...
     $(location).attr('href', '{{route('eac.portal.settings.manage.drug.dosage.concentration.index')}}') // <--- submit form programmatically
    });
   } else {
