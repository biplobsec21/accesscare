...
@section('content')
 <div class="titleBar">
  ...
  <h2 class="m-0">
   Country Manager <small>@yield('title')</small>
  </h2>
 </div><!-- end .titleBar -->
      ...
      @if($logData)
       @foreach($logData as $val)
        <tr data-id="{{$val->id}}">
         <td></td>
         <td>
          {{  \App\Country::where('id','=',$val->id)->firstOrFail()->name  }}
         </td>
         <td class="text-center">
          <span class="badge badge-secondary">{{ $val->total_action }}</span>
         </td>
         <td>
          <a href="{{ route('eac.portal.settings.manage.country.countrylogview',$val->id) }}" >
