...
@section('content')
 <div class="titleBar">
  ...
  <h2 class="m-0">
   Ethnicity Manager <small>@yield('title')</small>
  </h2>
 </div><!-- end .titleBar -->
 @php
 ...
 <div class="actionBar">
  ...
  <a href="{{ route('eac.portal.settings.manage.ethnicity.index') }}" class="btn btn-primary">
      ...
      @if($logData)
       @foreach($logData as $val)
        <tr data-id="{{$val->id}}">
         <td></td>
         <td>
          {{  \App\Ethnicity::where('id','=',$val->id)->firstOrFail()->name  }}
         </td>
         <td class="text-center">
          <span class="badge badge-secondary">{{ $val->total_action }}</span>
         </td>
         <td>          
          <a href="{{ route('eac.portal.settings.manage.ethnicity.statelogview',$val->id) }}" >
