		...
		<div class="row thisone m-0 mb-xl-5">
			<div class="col-sm-3 col-xl-auto mb-2 mb-sm-0 p-0">
				<div class="wizardSteps symbols nav flex-row flex-sm-column mt-sm-3" id="tab" role="tablist" aria-orientation="vertical">
					@php $i = 0 @endphp @foreach($permissions as $area => $sections)
					<a class="nav-link @if($role->hasArea($area)) complete @endif " data-toggle="pill" href="#x{{strtolower($area)}}" role="tab" id="x{{strtolower($area)}}T" aria-controls="x{{strtolower($area)}}" aria-selected="false">
			...
			<div class="col-sm-9 col-xl p-0">
				<div class="card tab-content wizardContent" id="tabContent">
					@php $i = 0 @endphp @foreach($permissions as $area => $sections)
					<div class="tab-pane fade m-2" id="x{{strtolower($area)}}" role="tabpanel" aria-labelledby="x{{strtolower($area)}}-tab">
						...
						<table class="table table-sm" id="manage-role-table">
							...
							<tr>
								...
								<td>
									<input name="level[{{$area}}][{{$section}}][view]" data-row="{{$section}}" data-col="view" value="1" type="checkbox" {{$role->
									can([$area, $section, 'view']) ? 'checked' : ''}}>
									<div class="checkmark"></div>
								</td>
								<td>
									<input name="level[{{$area}}][{{$section}}][update]" data-row="{{$section}}" data-col="update" value="1" type="checkbox" {{$role->
									can([$area, $section, 'update']) ? 'checked' : ''}}>
									<div class="checkmark"></div>
								</td>
								<td>
									<input name="level[{{$area}}][{{$section}}][create]" data-row="{{$section}}" data-col="create" value="1" type="checkbox" {{$role->
									can([$area, $section, 'create']) ? 'checked' : ''}}>
									<div class="checkmark"></div>
								</td>
								<td>
									<input name="level[{{$area}}][{{$section}}][delete]" data-row="{{$section}}" data-col="delete" value="1" type="checkbox" {{$role->
									can([$area, $section, 'delete']) ? 'checked' : ''}}>
									<div class="checkmark"></div>
								</td>
							</tr>
							@endforeach
...
</form>	</div><!-- end .viewData -->@endsection@section('scripts')
<script>
