 ...
 <div class="titleBar">
  ...
  @include('include.alerts')
    ...
    <a href="{{ route('eac.portal.user.list.merge') }}" class="btn btn-primary">
      <i class="fal fa-code-merge"></i> List Merge Users
    </a>
	</div><!-- end .actionBar -->

	<div class="viewData">
  <form method="post" action="{{ route('eac.portal.user.list.mergepost') }}">
    {{ csrf_field() }}
       ...
       @foreach($rowsPrimary as $val)
         <div class="card card-body mb-1 pt-2">
          <p class="text-muted m-0">
           <span class="strong">Name:</span> {{ $val->full_name }}
          </p>
          <p class="text-muted m-0">
           <span class="strong">Email:</span> {!! $val->email !!}
          </p>
          <p class="text-muted m-0">
           <span class="strong">User Types:</span> {{ $val->type ? $val->type->name : ''}}
          </p>
          <p class="text-muted m-0">
           <span class="strong">User Status:</span> {{ $val->status ? $val->status : ''}}
          </p>
