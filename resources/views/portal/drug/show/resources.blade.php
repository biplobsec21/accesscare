	...
	<div class="col-auto">
		<div class="btn-group btn-group-toggle btn-group-sm" data-toggle="buttons">
			<label class="btn btn-secondary active btn-sm" onclick="showactiveOrAll(1)">
				<input type="radio" name="show_active" id="option1" value="1"
					   autocomplete="off"> View Active
			</label>
			<label class="btn btn-secondary  btn-sm" onclick="showactiveOrAll(0)">
				<input type="radio" name="show_active" id="option2" value="0"
      ...
      @if($resource->file_id)
        <small class="d-block ml-2">
         <a href="{{ route('eac.portal.file.download', $resource->file_id) }}" class="text-info">
          <i class="fal fa-download text-muted"></i> Document
         </a>
        </small>
      @endif
     </td>
		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer p-2 d-flex justify-content-between">
					...
					<a href="{{ route('eac.portal.file.download', $resource->id) }}" class="btn btn-info">
						Download
					</a>
				</div>
			</div>
