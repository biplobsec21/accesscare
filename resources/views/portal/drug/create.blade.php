				...
				<div class="col-sm-auto p-0 pr-sm-2 mb-2 mb-sm-0 ">
					<div class="wizardSteps nav flex-row flex-sm-column justify-content-center justify-content-sm-start"
						...
						<a class="nav-link" id="five-tab" data-toggle="pill" href="#five" role="tab"
						   aria-controls="five" aria-selected="true">
							<span>Availability</span>
						</a>
						<a class="nav-link" id="six-tab" data-toggle="pill" href="#six" role="tab"
						   aria-controls="six" aria-selected="true">
							<span>Website Description</span>
						</a>
						<a class="nav-link" id="two-tab" data-toggle="pill" href="#two" role="tab"
					...
					<div class="card h-100 tab-content wizardContent" id="tabContent">
						<div class="tab-pane fade show active" id="one" role="tabpanel" aria-labelledby="one-tab">
							<div class="card-body">
								...
								<h5 class="mb-3">
									Please enter <strong>drug details</strong>
								</h5>
								<div class="row">
									<div class="col-md mb-3">
										...
										<input type="text" name="name"
										       ...
										       value="{{ old('name') }}" placeholder="Drug Name">
									...
									<div class="col-md mb-3">
										...
										<input type="text" name="lab_name"
										       ...
										       value="{{ old('lab_name') }}" placeholder="Drug Lab Name">
										<div class="invalid-feedback">
									...
									<select
										...
										@if($companies->count() > 0)
											...
											@foreach($companies as $company)
												<option
													value="{{ $company->id }}" {{ old('company_id') && old('company_id') ==  $company->id ? 'selected' : ''}}>{{ $company->name }}</option>
											@endforeach
										@endif
										{{-- @endif --}}
									</select>
								...
								<div class="mb-3">
									...
									<textarea maxlength="3000" name="desc"
								...
								<div class="row">
									<div class="col-md mb-3">
										...
										<input type="file" name="drug_image"
										       class="form-control{{ $errors->has('drug_image') ? ' is-invalid' : '' }}">
										<div class="invalid-feedback">
						...
						<div class="tab-pane fade" id="five" role="tabpanel" aria-labelledby="five-tab">
							<div class="card-body">
								...
								<div class="mb-3">
									<select name="countries_available[]"
									        class="form-control select2 {{ $errors->has('countries_available') ? ' is-invalid' : '' }}"
									        multiple
									        data-actions-box="true">
										@foreach($countries as $country)
											<option
												value="{{ $country->id }}" {{ old('countries_available') && in_array($country->id, old('countries_available')) ? 'selected' : ''}}>{{ $country->name }}</option>
										@endforeach
									</select>
									<div class="invalid-feedback">
									...
									<label class="h5 mb-0 d-block mt-2" for="hide_countries">
										<input type="checkbox" name="hide_countries" id="hide_countries" value="1"
										       @if(old('hide_countries')) checked @endif /> Hide Countries on EAC
										website
									</label>
								</div>
								<div class="mb-3">
									<label class="h5 mb-0 d-block" for="pre_approval_req">
										<input type="checkbox" name="pre_approval_req" id="pre_approval_req" value="1"
										       @if(old('pre_approval_req')) checked @endif /> Pre-Approval is Required
									</label>
								</div>
								<div class="mb-3">
									<label class="h5 mb-0 d-block" for="ship_without_approval">
										<input type="checkbox" name="ship_without_approval" id="ship_without_approval"
										       value="1" @if(old('ship_without_approval')) checked @endif /> Ship
										Without Approval
									</label>
								</div>
								<div class="mb-5">
									<label class="h5 mb-0 d-block" for="allow_remote">
										<input type="checkbox" name="allow_remote" id="allow_remote" value="1"
										       @if(old('allow_remote')) checked @endif /> Allow Remote Visits
									</label>
								</div>
							</div>
						...
						<div class="tab-pane fade" id="six" role="tabpanel" aria-labelledby="six-tab">
							<div class="card-body">
								<h5 class="mb-3 label_required">
         Enter publicly facing information on EAC-Company website
								</h5>
								<textarea
									class="form-control basic-editor {{ $errors->has('short_desc') ? ' is-invalid' : '' }}"
									name="short_desc" rows="15">{{ old('short_desc') }}</textarea>
								<div class="invalid-feedback">
						...
						<div class="tab-pane fade" id="two" role="tabpanel" aria-labelledby="two-tab">
							<div class="card-body">
								<h5 class="mb-3">
									Establish at least 1 <strong>component</strong>
								</h5>
								<div class="input_fields_wrap mb-3">
									<div class="row ml-0 mb-1 mr-0">
										<div class="col pl-0">
											<input type="text"
											       class="form-control{{ $errors->has('component_main') ? ' is-invalid' : '' }}"
											       name="component_main" placeholder="Component 1"
											       value="{{ old('component_main') ? old('component_main') : 'Component 1' }}"/>
										</div>
										<div class="col-auto pr-0">
											<a href="#" class=" btn btn-success" data-toggle="modal"
											   data-target="#newDosageModal">
												Add Dosages</a>
										</div>
									</div>
									<div class="invalid-feedback">
									...
									@if(Session::get('components'))
										...
										<div class="input-group">
											<input class="form-control" type="text" placeholder="Component {{$i+2}}"
											       name="component_name[]" value="{{$components[$i]}}"/>
								...
								<div class="mb-5">
									<button class="add_field_button btn btn-info btn-sm window-btn">
										<span class="fal fa-plus"></span> Add Component
									</button>
								</div>
							</div>
							<div class="card-footer d-flex justify-content-between p-2">
				...
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content">
						<div class="modal-header p-2">
							<h5 class="mb-3">
								Enter information on drug component
							</h5>
							...
							<ul class="list-group list-group-flush">
								<li class="list-group-item border-secondary pl-0 pr-0">
									...
									<div class="mb-3">
										...
										<select class="form-control {{ $errors->has('form_id') ? ' is-invalid' : '' }}"
											...
											@if($dosageForm->count() > 0)
												@foreach($dosageForm as $val)
													<option
														value="{{ $val->id }}" {{ old('form_id') == $val->id ? 'selected' : '' }} >{{$val->name}}</option>
												@endforeach
											@endif
										</select>
										...
										<label class="h5 mb-0 d-block mt-2" for="active">
											<input type="checkbox" name="active"
											       id="active" {{ old('active') ? 'checked' : '' }} /> Dosage is
											Available/Active <!-- changed from 'Status' -->
										</label>
									</div>
									<div class="row m-0 mb-3">
										<div class="col p-0">
											...
											<input type="number"
											       ...
											       name="amount" step="any" value="{{ old('amount')}}">
											<div class="invalid-feedback">{{ $errors->first('amount') }}</div>
										</div>
										<div class="col p-0">
											<label class="d-block label_required">Unit</label>
											<select
												class="form-control border-left-0 {{ $errors->has('unit_id') ? ' is-invalid' : '' }}"
												name="unit_id">
												<option disabled="" hidden="" selected="" value="">-- Select --</option>
												@if($dosageUnit->count() > 0)
													@foreach($dosageUnit as $val)
														<option
															value="{{ $val->id }}" {{ old('unit_id') == $val->id ? 'selected' : '' }} >{{$val->name}}</option>
													@endforeach
												@endif
											</select>
											<div class="invalid-feedback">
									...
									<div class="row">
											...
											<select
												...
												@if($dosageStrength->count() > 0)
													@foreach($dosageStrength as $val)
														<option
															value="{{ $val->id }}" {{ old('strength_id') == $val->id ? 'selected' : '' }} >{{$val->name}}</option>
													@endforeach
												@endif
											</select>
											<div class="invalid-feedback">
										...
										<div class="col-md mb-3">
											<label class="d-block label_required">
												Optimal Temp<span class="d-none d-lg-inline">erature</span>
											</label>
           <div class="input-group">
             ...
             <span class="input-group-text d-flex flex-column align-items-stretch text-dark">
              ...
              <small class="d-flex"><input type="radio" name="temp_opt" value="F" /> F</small>
						...
						<div class="modal-footer p-2 d-flex justify-content-between">
							<button type="button" class="btn btn-secondary cancel" name="cancel" value="cancel"
