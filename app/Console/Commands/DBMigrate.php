...
class DBMigrate extends Command
{
	...
	public function handle()
	{
		...
		$this->info('Retrieving States...');
		$this->handleStates();
		$this->info('States Migrated!');

		//$this->info('Retrieving Pages...');
		//$this->handlePages();
		//$this->info('Pages Migrated!');

		$this->info('Retrieving Companies...');
		$this->handleCompanies();
		$this->info('Companies Migrated!');

//		$this->info('Retrieving Abilities...');
//		//$this->handleAbilities();
//		$this->info('Abilities Configured!');

//		$this->info('Retrieving Roles...');
//		$this->handleRoles();
...
//		$this->info('Retrieving Mailers...');
//		$this->handleMailers();
//		$this->info('Mailers Migrated!');
//
//		$this->info('Retrieving Shipping Couriers...');
//		$this->handleShippingCourier();
//		$this->info('Shipping Couriers Migrated!');

		$this->info('Retrieving Dosage Units...');
		$this->handleDosageUnit();
		$this->info('Dosage Units Migrated!');

		$this->info('Retrieving Concentration Units...');
		$this->handleConcentrationUnit();
		$this->info('Concentration Units Migrated!');
		...
		$this->info('Retrieving Routes Of Administration...');
		$this->handleAdministrationRoutes();
		$this->info('Routes Of Administration Migrated!');


		$this->info('Retrieving Dosage Strengths...');
		$this->handleDosageStrength();
		$this->info('Dosage Strengths Migrated!');

		$this->info('Retrieving Dosage Forms...');
		$this->handleDosageForm();
		$this->info('Dosage Forms Migrated!');
...
//		$this->info('Notifications Migrated!');
		...
		$this->info('Rid not approved reason');
		$this->handleRidNotApprovedReason();
		$this->info('Rid not approved reason!');
...
//		$this->info('Physician user set physician delegate type done !');
	...
	{
		//$tables = DB::connection('mysql_dest')->select('SHOW TABLES');
		$tables = [
			'users', 'user_certifications', 'user_groups', 'rid_user_group', 'drug_user_group', 'notifications',
			'rids', 'rid_records', 'rid_documents', 'rid_regimens', 'rid_shipments',
			'drug', 'drug_components', 'drug_documents', 'drug_supply', 'dosages', 'dosage_forms', 'dosage_routes', 'dosage_strength', 'dosage_units', 'resources',
			'drug_lots', 'depots', 'pharmacies', 'pharmacists', 'document_types', 'countries', 'states', 'companies', 'departments', 'concentration_units',
			'merge_data', 'DO_NOT_TOUCH_DEV_UPDATE_SCRIPT_TABLE_DO_NOT_TOUCH', 'addresses', 'log', 'rid_not_approved_reasons', 'files'
		];
		$colname = 'Tables_in_' . env('DB_DATABASE_DEST');
		try {
			foreach ($tables as $table) {
				DB::connection('mysql_dest')->table($table)->truncate();
			}
		} catch (\Exception $e) {
			die ($e);
		}
	...
	{
		$countries = DB::connection($this->from['name'])->table('country')->orderBy('country_name');
	...
	{
		$companies = DB::connection($this->from['name'])->table('company');
		...
		foreach ($companies as $company) {
			...
			if (!is_null($company->company_phone_main)) {
				$companyMainPhone = (PhoneNumberUtil::getInstance()->format(PhoneNumberUtil::getInstance()->parse($company->company_phone_main, "US"), PhoneNumberFormat::E164));
				...
				try {
					...
					$companyPhoneMain->country_id = Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $company->country_id, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id;
			...
			if (is_null($company->company_addr_1) && $company->company_addr_1 == 'TBD' || $company->company_addr_1 == 'unknown') {
				...
				try {
					...
					if (strlen($company->company_state) == 2) {
						...
						$companyAddress->state_province = State::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $company->company_state, 'table_name' => 'states'])->first()->id_new))->firstOrFail()->id;
					}
					// dd(Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $company->country_id, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id);
					$companyAddress->country_id = Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $company->country_id, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id;
			...
			$companyModel->country_id = Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $company->country_id, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id;
			// dd($companyModel->country_id);
	...
	protected function handleUsers()
	{
		...
		foreach (json_decode(json_encode($users)) as $user) {
			...
			if ($user->is_company) {
				...
				$userModel->company_id = DEVUPDATESCRIPTTABLE::where(['table_name' => 'company', 'id_old' => $user->company_id])->firstOrFail()->id_new;
			}
		...
		$userModel = new User();
		$userModel->id = $this->newID(User::class);
		$userModel->first_name = 'Biplob';
		$userModel->last_name = 'Hossain';
		$userModel->status = 'Approved';
		$userModel->type_id = UserType::where('name', 'Early Access Care')->first()->id;
		$userModel->email = 'hossainb@quasars.com';
		$userModel->password = '$2y$10$51Dyz7nP6l49FLYEOqJBXe05YRNJIkZJC3FjBmEFPZ8caHCjiWa3i';
		$userModel->is_developer = 1;
		$userModel->saveOrFail();
	...
	{
		$eacUser = DB::connection($this->from['name'])->table('eac_user');
		$physicianUser = DB::connection($this->from['name'])->table('physician_user');
		$companyUser = DB::connection($this->from['name'])->table('company_user');

		$normalized = [];
		foreach ($eacUser as $user) {
			$normalized[count($normalized)] = ['id' => $user->eac_user_id, 'type' => 'eac', 'title' => null, 'first_name' => trim($user->eac_user_fname), 'last_name' => trim($user->eac_user_lname), 'phone' => $this->normalizePhone($user->eac_user_phone), 'email' => strtolower($user->eac_user_email), 'role' => $user->eac_user_role_id, 'address' => ['addr1' => $user->eac_user_addr_1, 'addr2' => $user->eac_user_addr_2, 'city' => $user->eac_user_city, // 'state' => (strlen($user->eac_user_state) == 2) ? State::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->eac_user_state, 'table_name' => 'states'])->firstOrFail()->id_new))->firstOrFail()->id : $user->eac_user_state,
				'state' => (strlen($user->eac_user_state) == 2) ? State::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->eac_user_state, 'table_name' => 'states'])->firstOrFail()->id_new))->firstOrFail()->id : null, 'zipcode' => $user->eac_user_zip, 'country' => null,], 'is_company' => 0, 'status' => 'Approved', 'is_active' => ($user->eac_user_active == 1) ? 1 : 0, 'created_at' => $user->eac_user_added, 'updated_at' => $user->eac_user_updated,
				'password' => $user->eac_user_password];
		}

		foreach ($physicianUser as $user) {
			$normalized[count($normalized)] = ['id' => $user->physician_user_id, 'type' => 'phy', 'parent_id' => $user->physician_user_title, 'title' => $user->physician_user_title, 'first_name' => trim($user->physician_user_fname), 'last_name' => trim($user->physician_user_lname), 'phone' => $this->normalizePhone($user->physician_user_phone), 'email' => strtolower($user->physician_user_email), 'role' => $user->physician_user_type_id, 'address' => ['addr1' => $user->physician_user_addr_1, 'addr2' => $user->physician_user_addr_2, 'city' => $user->physician_user_city, // 'state' => (strlen($user->physician_user_state) > 2) ? State::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->physician_user_state, 'table_name' => 'states'])->firstOrFail()->id_new))->firstOrFail()->id : $user->physician_user_state,
				'state' => (strlen($user->physician_user_state) > 2) ? State::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->physician_user_state, 'table_name' => 'states'])->firstOrFail()->id_new))->firstOrFail()->id : null, 'zipcode' => $user->physician_user_zip, // 'country' => (strlen($user->physician_user_country) > 2) ? Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->physician_user_country, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id : $user->physician_user_country,
				'country' => (strlen($user->physician_user_country) > 2) ? Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->physician_user_country, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id : null,], 'is_company' => 0, 'status' => $user->physician_user_status, 'is_active' => $user->physician_user_active, 'created_at' => $user->physician_user_added, 'updated_at' => $user->physician_user_updated,
				'password' => $user->physician_user_password];
		}

		foreach ($companyUser as $user) {
			$normalized[count($normalized)] = ['id' => $user->company_user_id, 'type' => 'com', 'title' => $user->company_user_title, 'first_name' => trim($user->company_user_fname), 'last_name' => trim($user->company_user_lname), 'phone' => $this->normalizePhone($user->company_user_phone), 'email' => strtolower($user->company_user_email), 'role' => $user->company_user_type_id, 'address' => ['addr1' => $user->company_user_addr_1, 'addr2' => $user->company_user_addr_2, 'city' => $user->company_user_city, // 'state' => (strlen($user->company_user_state) > 2) ? State::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->company_user_state, 'table_name' => 'states'])->firstOrFail()->id_new))->firstOrFail()->id : $user->company_user_state,
				'state' => (strlen($user->company_user_state) > 2) ? State::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->company_user_state, 'table_name' => 'states'])->firstOrFail()->id_new))->firstOrFail()->id : null, 'zipcode' => $user->company_user_zip, // 'country' => (strlen($user->company_user_country) > 2) ? Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->company_user_country, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id : $user->company_user_country,
				'country' => (strlen($user->company_user_country) > 2) ? Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $user->company_user_country, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id : null,], 'is_company' => 1, 'company_id' => $user->company_id, 'status' => 'Approved', 'is_active' => $user->company_user_active, 'created_at' => $user->company_user_added, 'updated_at' => $user->company_user_updated,
	...
	public function handleDrugDocumentType()
	{
		...
		$drugDocumentTypes = DB::connection($this->from['name'])->table($oldTable);
		// dd($drugDocumentTypes);
		$bar = $this->output->createProgressBar(count($drugDocumentTypes));
		$bar->setBarWidth(2000);
		$bar->start();
	...
	{
		$oldTable = 'drug';
		$drugs = DB::connection($this->from['name'])->table($oldTable);
		// dd($drugs);
	...
	public function handleResourceTypes()
	{
		...
		$resourceTypes = DB::connection($this->from['name'])->table($oldTable);
		// dd($resourceTypes);
	...
	{
		$oldTable = 'drug_res';
		$drugResources = DB::connection($this->from['name'])->table($oldTable);
		// dd($drugResources);
		...
		foreach ($drugResources as $drugResource) {
			...
			$resourceModel->drug_id = Drug::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'drug', 'id_old' => $drugResource->drug_id])->firstOrFail()->id_new)->firstOrFail()->id;
			$resourceModel->type_id = DocumentType::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'drug_res_type', 'id_old' => $drugResource->drug_res_type_id])->firstOrFail()->id_new)->firstOrFail()->id;
	...
	protected function handleAdministrationRoutes()
	{
		...
		$administrationRoutes = DB::connection($this->from['name'])->table($oldTable);
	...
	{
		$oldTable = 'drug_doc';
		$drugDocuments = DB::connection($this->from['name'])->table($oldTable);
		// dd($drugDocuments);
		...
		foreach ($drugDocuments as $drugDocument) {
			...
			try {
				$drugDocumentModel->type_id = DocumentType::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'drug_doc_type', 'id_old' => $drugDocument->drug_doc_type_id])->firstOrFail()->id_new)->firstOrFail()->id;
			} catch (\Exception $e) {
	...
	protected function handleDrugDosage()
	{
		...
		$drugDosages = DB::connection($this->from['name'])->table($oldTable);
		// dd($drugDosages);
		...
		foreach ($drugDosages as $drugDosage) {
			...
			$drugDosageModel->form_id = DosageForm::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'dosage_form', 'id_old' => $drugDosage->dosage_form_id])->firstOrFail()->id_new)->firstOrFail()->id;//DocumentType::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'drug_doc_type', 'id_old' => $drugDosage->drug_doc_type_id])->firstOrFail()->id_new)->firstOrFail()->id;
			//			$drugDosageModel->route_id = 'd';//DosageRoute::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'route_admin', 'id_old' => $drugDosage->drug_dosage_route_id])->firstOrFail()->id_new)->firstOrFail()->id;
			$drugDosageModel->strength_id = DosageStrength::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'formulation_strength', 'id_old' => $drugDosage->drug_dosage_strength])->firstOrFail()->id_new)->firstOrFail()->id;
			...
			$drugDosageModel->active = 1;//$drugDosage->drug_doc_desc;
			$drugDosageModel->created_at = $drugDosage->drug_dosage_added;
			$drugDosageModel->updated_at = $drugDosage->drug_dosage_updated;
			try {
				$drugDosageModel->unit_id = DosageUnit::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'dosage_unit', 'id_old' => $drugDosage->dosage_unit_id])->firstOrFail()->id_new)->firstOrFail()->id;
			} catch (\Exception $e) {
	...
	{
		$oldTable = 'rid';
		$old_rid_ids = DB::connection($this->from['name'])->table($oldTable)->select('rid_id')->whereNull('parent_id')->pluck('rid_id');

		$old_resupplys = DB::connection($this->from['name'])->table($oldTable)->select('rid_id')->whereNotNull('parent_id')->pluck('rid_id');
	...
	protected function createRidMaster($old_id)
	{
		...
		$ridModel->username = $this->generateUnique('mysql', Rid::getTableName(), 'username', $this->GENERATESSTRINGS_CHARS_UPPER_LOWER_NUM, 15);
		$ridModel->password = $this->makeHash($rid->rid_password);
	...
	{
		$oldTable = 'rid_ship';
		$ridShipments = DB::connection($this->from['name'])->table($oldTable);
			...
			$ridVisit->saveOrFail();
				...
				if (!is_null($rid->rid_pharmacist_phone)) {
					...
					if ($userPhone) {
						...
						$ridPhone->country_id = (is_null($rid->country_id)) ? null : Country::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'country', 'id_old' => $rid->country_id])->firstOrFail()->id_new)->firstOrFail()->id;
				...
				if (is_null($rid->rid_shipment_address) || $rid->rid_shipment_address == 'TBD' || $rid->rid_shipment_address == 'unknown') {
					...
					if ($userAddr) {
						...
						$ridPharmacyAddress->state_province = (is_null($rid->state_id)) ? null : State::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'states', 'id_old' => $rid->state_id])->firstOrFail()->id_new)->firstOrFail()->id;
						$ridPharmacyAddress->country_id = (is_null($rid->country_id)) ? null : Country::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'country', 'id_old' => $rid->country_id])->firstOrFail()->id_new)->firstOrFail()->id;
				...
				if (is_null($ridShipment->rid_ship_originate_address) && $ridShipment->rid_ship_originate_address == 'TBD' || $ridShipment->rid_ship_originate_address == 'unknown') {
					...
					if (!is_null($getExistingAddress) && isset($getExistingAddress->addr1)) {
						...
						try {
							$depotAddress->country_id = Country::where('id', '=', (DEVUPDATESCRIPTTABLE::where(['id_old' => $ridShipment->rid_ship_originate_country, 'table_name' => 'country'])->firstOrFail()->id_new))->firstOrFail()->id;
						} catch (\Exception $e) {
	...
	protected function handleRidDocuments()
	{
		...
		$resupplyDocuments = DB::connection($this->from['name'])->table($oldTable);
	...
	protected function handleRIDUsers()
	{
		...
		$ridUsers = DB::connection($this->from['name'])->table($oldTable);
		$bar = $this->output->createProgressBar(count($ridUsers));
		$bar->setBarWidth(2000);
		$bar->start();
		foreach ($ridUsers as $ridUser) {
			$__IDLOGGER = new DEVUPDATESCRIPTTABLE();
			$ridUserModel = new RidUser();

			$__IDLOGGER->table_name = $oldTable;
			$__IDLOGGER->id_old = $ridUser->physician_user_rid_id;

			try {
				$ridUserModel->id = $__IDLOGGER->id_new = $this->newID(RidUser::class);
				$ridUserModel->user_id = User::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'physician_user', 'id_old' => $ridUser->physician_user_rid_assigned_to])->firstOrFail()->id_new)->firstOrFail()->id;
				$ridUserModel->rid_id = Rid::where('id', '=', DEVUPDATESCRIPTTABLE::where(['table_name' => 'rid', 'id_old' => $ridUser->rid_id])->firstOrFail()->id_new)->firstOrFail()->id;
			} catch (ModelNotFoundException $notFoundException) {
				continue 1;
			}

			try {
				$ridUserModel->role_id = Role::where('id_old', $ridUser->physician_user_type_id)->firstOrFail()->id;
			} catch (\Exception $e) {
				$ridUserModel->role_id = null;
			}
			$__IDLOGGER->saveOrFail();
	...
	{
		$oldTable = 'rid_note';
		$notes = DB::connection($this->from['name'])->table($oldTable);
	...
	protected function handleNotifications()
	{
		...
		$notifications = DB::connection($this->from['name'])->table($oldTable)->limit(100);
		...
		foreach ($notifications as $notification) {
			...
			try {
				...
				$notificationModel->user_id = null;
			}
			// get subject id
			try {
	...
	protected function handleUserCertificates()
	{
		...
		$physicanUser = DB::connection($this->from['name'])->table($oldTable);
		// dd($physicanUser);
